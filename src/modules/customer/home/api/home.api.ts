...
export const getAllCommerces = async (
  ...
  try {
    const { data } = await httpClient.get<
      PaginatedResponse<CommercePublicResponse>
    >(`/api/v1/public/commerces`, { params: { locality, page, size } });
    return data;
  } catch (error) {
...
export const getCommerceDetail = async (
  commerceId: string,
...
export const getProductsByCommerceTypeForClient = async (
  ...
  try {
    ...
    console.error(
      "Error fetching products by commerce type for client:",
      error,
    );
    return [];
  }
